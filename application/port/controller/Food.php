...
class Food extends Controller
{
	...
	public function FoodInfoData()
	{
		...
		$UserInfo = DB::name("user")->where("uid",$data['uid'])->field("truename,nickname,headimgurl")->find();
	...
	public function GetLeave() {
		$data = input("param.") ? input("param.") : '';
		// var_dump($data['email']);die;
		$food = new FoodModel();
		if(!isset($data['email']) && empty($data['email']) || !isset($data['content']) && empty($data['content'])) {
			return json_encode(['code'=>1002,'msg'=>'Parameter Error']);
		}
		$res = $food->GetLeave($data);
